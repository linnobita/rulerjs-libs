...
export default class RulerjsLib {
  ...
  public getGuideLines(): Array<GuideLineInfo> {
    return this.guides.map((guide: GuideLine) => {
      return {
        point: {
          x: Math.round((parseInt(guide.line.style.left || '0', 10) - this.curDeltaX - this.options.rulerHeight) * this.curScale),
          y: Math.round((parseInt(guide.line.style.top || '0', 10) - this.curDeltaY - this.options.rulerHeight) * this.curScale)
        },
  ...
  public setScale(scale: number): void {
    ...
    for (const guide of this.guides) {
      ...
      if (guide.direction === UTILS.RulerDirection.HORIZONTAL) {
        ...
        guide.line.style.top =
          UTILS.pixelize((curPos - origDelta - this.curDeltaY) / curScaleFactor + origDelta + this.curDeltaY);
      } else {
        curPos = parseInt(guide.line.style.left || '0', 10);
        guide.line.style.left =
          UTILS.pixelize((curPos - origDelta - this.curDeltaX) / curScaleFactor + origDelta + this.curDeltaX);
      }
  ...
  public destroy() {
    ...
    for (const ruler of this.rulers) {
      this.options.container.removeEventListener('mousemove', (ev: MouseEvent) => this.rulerMouseMove(ev, ruler));
      this.options.container.removeEventListener('mousedown', (ev: MouseEvent) => this.rulerMouseDown(ev, ruler));
      ruler.tracker.parentNode && ruler.tracker.parentNode.removeChild(ruler.tracker);
    }
    for (const corner of this.corners) {
      corner.destroy();
    }
    this.options.container.removeEventListener('mouseup', (ev: MouseEvent) => this.containerMouseUp(ev, this));
  ...
  private generateRulers(): void {
    ...
    this.rulerEle = this.options.measuringTarget !== null ?
    ...
    this.options.container.addEventListener('mouseup', (ev: MouseEvent) => this.containerMouseUp(ev, this));
  }

  private createRuler(alignment: UTILS.RulerAlignment): Ruler {
    const direction = (alignment === UTILS.RulerAlignment.LEFT || alignment === UTILS.RulerAlignment.RIGHT) ?
      UTILS.RulerDirection.VERTICAL : UTILS.RulerDirection.HORIZONTAL;
    const rulerStyle = direction === UTILS.RulerDirection.VERTICAL ? 'rulerjs_ruler_Vertical' : 'rulerjs_ruler_Horizontal';
    ...
    this.options.container.addEventListener('mousemove', (ev: MouseEvent) => this.rulerMouseMove(ev, newRuler));
    newRuler.canvas.addEventListener('mousedown', (ev: MouseEvent) => this.rulerMouseDown(ev, newRuler));

    newRuler.drawRuler(Math.max(this.rulerEle.clientWidth, this.rulerEle.clientHeight), this.options.rulerHeight);
  ...
  private createGuideLine(direction: UTILS.RulerDirection, x: number, y: number, ev?: MouseEvent, isSet: boolean = false): void {
    ...
    const guideStyle = direction === UTILS.RulerDirection.VERTICAL ? 'rulerjs_lineVertical' : 'rulerjs_lineHorizontal';
    ...
    if (direction === UTILS.RulerDirection.VERTICAL) {
      guide.style.left = UTILS.pixelize(!isSet ?
      ...
      guide.style.top = UTILS.pixelize(!isSet ?
    ...
    this.guides.push(new GuideLine(
      this.options,
      {
        line: guide,
